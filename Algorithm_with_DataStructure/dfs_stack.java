 ...
 * 가능한 깊게 노드를 탐색하는 방법입니다. 스택(Stack) 또는 재귀 함수를 사용하여 구현할 수 있습니다.
 *
 * 2. 입력과 출력:
 * - 입력: 그래프의 시작 노드
 ...
 *
 * 5. 알고리즘 세부 동작:
 * - 시작 노드를 스택에 넣고, 방문했다는 표시를 합니다.
 * - 스택에서 노드를 하나 꺼내고, 해당 노드의 인접 노드 중 방문하지 않은 노드를 모두 스택에 넣고, 방문했다는 표시를 합니다.
 * - 스택이 비어있지 않다면, 위의 과정을 반복합니다.
 * - 스택이 비어있다면, 알고리즘을 종료합니다.
 *
 * 도식화:
...
public class DFSExample {
    ...
    public static void dfs(int start) {
        ...
        // 스택이 비어있지 않는 한 계속해서 탐색
        while (!stack.isEmpty()) {
            // 스택의 최상단 원소를 확인
            int x = stack.peek();
            boolean isUnvisitedNodeExist = false;
            ...
            for (int i = 0; i < graph.get(x).size(); i++) {
                ...
                if (!visited[y]) {
                    ...
                    visited[y] = true;
                    System.out.print(y + " ");
                    isUnvisitedNodeExist = true;
                    break;
                }
            }

            // 방문하지 않은 인접 노드가 없다면 스택에서 최상단 노드를 제거
            if (!isUnvisitedNodeExist) {
                stack.pop();
            }
        }
    }
